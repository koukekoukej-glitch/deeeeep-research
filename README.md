...
端到端深度研究 Skill。四阶段流水线：需求对齐 → 多 Agent 并行数据收集 → 证据交叉验证 → 报告生成，产出带完整引用的结构化研究报告。
...
│
Phase 4 报告生成 ── 结构化报告 + sources.json，角标一致性验证
```
...
按格式契约生成报告和 `sources.json`，验证正文角标与来源条目的一致性，交付文件链接。
